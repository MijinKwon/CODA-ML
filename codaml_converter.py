...
def format_handside(ku, hs_name, ob_hs):
    dict_entity_type = {'PH': 'Phenotype', 'BP': 'Biological_Process', 'MF': 'Molecular_Function', 'CP': 'Compound_Product', 'GP': 'Gene_Product'}
    ...
    for ob_entity in ob_hs:
        ...
        for one_entity in ob_entity.entitycore.split('&'):
            ...
            if molecule_type != 'NA' or mods != 'NA' or variant != 'NA':
                ...
                if variant != 'NA':
                    ET.SubElement(mole_spec, 'Molecule_Isoform').text = variant
        ...
        if any([antm_text != '' for antm_text in list_antm]):
            ...
            for antm_name, antm_text in zip(['Organ', 'Tissue', 'Cell'], list_antm):
                if antm_text != '':
                    ET.SubElement(antm, antm_name).text = antm_text
...
def format_asso_cnxt(ku, dict_asso_cntx):
    if dict_asso_cntx != {}:
        ...
        for key, list_value in dict_asso_cntx.items():
            if key == 'Cell Line':
                if list_value != 'NA':
                    append_item(asso_cntx, 'Cell line', list_value)

            else:
                for value in list_value:
                    if value != 'NA':
                        append_item(asso_cntx, key, value)
...
def format_score(ku, str_ev_score):
    ...
    ku.append(ev_score)
    key, value = str_ev_score.split(':')
    append_item(ev_score, key, value)


def append_item(parent, key, value):
...
def convert_to_xml(f_in_name, f_out_name):
    ...
    for line in f_in:
        ...
        format_handside(ku, 'LHS', ob_coda.lt)
        ET.SubElement(ku, 'Association').text = str(ob_coda.asso)
        format_handside(ku, 'RHS', ob_coda.rt)
        format_asso_cnxt(ku, ob_coda.assoCntx)

        if str(ob_coda.assoInSource) != 'NA':
            ET.SubElement(ku, 'Association_In_Source').text = str(ob_coda.assoInSource)

        ET.SubElement(ku, 'Species').text = str(ob_coda.spec)
...
if __name__ == '__main__':
    ...
    convert_to_xml('codaml_converter_input.tsv', 'codaml_examples.xml')
